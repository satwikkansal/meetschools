...
query_result = google_places.nearby_search(
        ...
        radius=25000, types=[types.TYPE_SCHOOL])
...
for place in query_result.places:
   ...
   # print place.website
    ...
    try:
    	total_reviews = len(place.details['reviews'])
    except:
	pass
    score = 0
    for rev in place.details['reviews']:
		try:
		    reviews[str(place.place_id)] = ([rev['rating'],rev['text'],rev['author_name']])
		    res = requests.post(sentiment_url+str(rev['text']),{'apikey':'36a143cf-7451-4246-b245-0ac8ddd104ab'})
		    res_dict = json.loads(res.text)
		    score +=res_dict['aggregate']['score']
		except:
		    pass
    positivity = float(score/total_reviews)*100
    print positivity
    
    try:
	    schools.append({'name': place.name, 'website': place.website, 'latitude': str(place.geo_location),
		           'id': place.place_id, 'phone': place.international_phone_number, 'address': place.formatted_address
		              , 'photo': place.photos[0].url,'positivity':positivity,'reviews':str(place.details['reviews'])})
	    print schools
    except:
	    pass
    with open('sentiment_analysis.txt', 'wb') as outfile:
