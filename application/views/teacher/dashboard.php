
<link rel="stylesheet" href="/static/css/pages/shortcode_timeline2.css">
<link rel="stylesheet" href="/static/plugins/scrollbar/css/jquery.mCustomScrollbar.css">
<link rel="stylesheet" href="/static/css/pages/profile.css">


<!--=== Profile ===-->
<div class="container content profile">
    <div class="row">
        <!--Left Sidebar-->
        <div class="col-md-3 md-margin-bottom-40">
            <img class="img-responsive profile-img margin-bottom-20" src="https://pbs.twimg.com/profile_images/490643057822273537/pMkrGQPT_400x400.jpeg" alt="">

            <ul class="list-group sidebar-nav-v1 margin-bottom-40" id="sidebar-nav-1">
                <li class="list-group-item active">
                    <a href="/teacher/dashboard"><i class="fa fa-bar-chart-o"></i> Dashboard</a>
                </li>
                <li class="list-group-item">
                    <a href="/teacher/add_homework"><i class="fa fa-user"></i> Add Homework</a>
                </li>
                <li class="list-group-item">
                    <a href="/teacher/fellow_teachers"><i class="fa fa-group"></i> Fellow Teachers</a>
                </li>
                <li class="list-group-item">
                    <a href="/teacher/my_students"><i class="fa fa-cubes"></i> My Students</a>
                </li>
                <li class="list-group-item">
                    <a href="/teacher/complaints"><i class="fa fa-comments"></i> Complaints</a>
                </li>
                <li class="list-group-item">
                    <a href="/teacher/suggestions"><i class="fa fa-history"></i> Suggestions</a>
                </li>
                <li class="list-group-item">
                    <a href="/teacher/settings"><i class="fa fa-cog"></i> Settings</a>
                </li>
            </ul>

           

        </div>
        <!--End Left Sidebar-->

        <!-- Profile Content -->
        <div class="col-md-9">
            <div class="profile-body">
                <div class="profile-bio">
                    <div class="row">
                        <div class="col-md-5">
                            <img class="img-responsive md-margin-bottom-10" src="https://pbs.twimg.com/profile_images/490643057822273537/pMkrGQPT_400x400.jpeg" alt="">
                            <a class="btn-u btn-u-sm" href="#">Change Picture</a>
                        </div>
                        <div class="col-md-7">
                            <h2>Mrs. Kanika Jain</h2>
                            <span><strong>Subjects:</strong> Biology</span>
                            <span><strong>Classes:</strong> XI-XII</span>
                            <span><strong>Qualification:</strong> MSc. (Biology)</span>
                           
                        </div>
                    </div>
                </div><!--/end row-->

                <hr>
                
                <div class="panel panel-profile" id="teachers">
                    <div class="panel-heading overflow-h">
                        <h2 class="panel-title heading-sm pull-left"><i class="fa fa-university"></i> Fellow Teachers</h2>
<a href="/teacher/fellow_teachers" class="btn-u btn-brd btn-brd-hover btn-u-dark btn-u-xs pull-right">View All</a>
                    </div>
                    <div class="panel-body">
                        <div class="table-search-v2">
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Image</th>
                                            <th class="hidden-sm">About</th>
                                            <th>Subject</th>
                                            <th>Contacts</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <td>
                                                <img class="rounded-x" src="http://www.dpsrkp.net/images/faculty/biology/IMG_3944.JPG" alt="">
                                            </td>
                                            <td class="td-width">
                                                <h3><a href="#">Mrs. Niva Chhonkar</a></h3>
                                                <p><b>Classes to which subject is taught:</b> VI to XII</p>
                                                
                                            </td>
                                            <td>
                                                Biology
                                            </td>
                                            <td>
                                                <ul class="list-inline s-icons">
                                                    <li>
                                                        <a data-placement="top" data-toggle="tooltip" class="tooltips" data-original-title="Facebook" href="#">
                                                            <i class="fa fa-facebook"></i>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a data-placement="top" data-toggle="tooltip" class="tooltips" data-original-title="Twitter" href="#">
                                                            <i class="fa fa-twitter"></i>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a data-placement="top" data-toggle="tooltip" class="tooltips" data-original-title="Dropbox" href="#">
                                                            <i class="fa fa-dropbox"></i>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a data-placement="top" data-toggle="tooltip" class="tooltips" data-original-title="Linkedin" href="#">
                                                            <i class="fa fa-linkedin"></i>
                                                        </a>
                                                    </li>
                                                </ul>
                                                <span><a href="#">niva.chhonkar@gmail.com</a></span>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <img class="rounded-x" src="http://www.dpsrkp.net/images/faculty/accountancy/RT60.JPG" alt="">
                                            </td>
                                            <td>
                                                <h3><a href="#">Mr. Shyam Sunder</a></h3>
                                                <p><b>Classes to which subject is taught:</b> XI to XII</p>
                                            </td>
                                            <td>
                                                Accountancy
                                            </td>
                                            <td>
                                                <ul class="list-inline s-icons">
                                                    <li>
                                                        <a data-placement="top" data-toggle="tooltip" class="tooltips" data-original-title="Facebook" href="#">
                                                            <i class="fa fa-facebook"></i>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a data-placement="top" data-toggle="tooltip" class="tooltips" data-original-title="Twitter" href="#">
                                                            <i class="fa fa-twitter"></i>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a data-placement="top" data-toggle="tooltip" class="tooltips" data-original-title="Dropbox" href="#">
                                                            <i class="fa fa-dropbox"></i>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a data-placement="top" data-toggle="tooltip" class="tooltips" data-original-title="Linkedin" href="#">
                                                            <i class="fa fa-linkedin"></i>
                                                        </a>
                                                    </li>
                                                </ul>
                                                <span><a href="#">shyamteacher@hotmail.com</a></span>
                                                
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div>
                
                
                <hr />

                <div class="panel panel-profile" id="alumini">
                    <div class="panel-heading overflow-h">
                        <h2 class="panel-title heading-sm pull-left"><i class="fa fa-graduation-cap"></i> My Students</h2>
                        <a href="/teacher/my_students" class="btn-u btn-brd btn-brd-hover btn-u-dark btn-u-xs pull-right">View All</a>
                    </div>
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="profile-blog blog-border">
                                    <img class="rounded-x" src="https://cdn2.iconfinder.com/data/icons/professions/512/student_graduate_boy_profile-512.png" alt="">
                                    <div class="name-location">
                                        <strong>Ankur Jain</strong>
                                        <span><i class="fa fa-map-marker"></i><a href="#">Delhi</a></span>
                                    </div>
                                   
                                </div>
                            </div>

                            <div class="col-sm-6">
                                <div class="profile-blog blog-border">
                                    <img class="rounded-x" src="https://cdn2.iconfinder.com/data/icons/professions/512/student_graduate_boy_profile-512.png" alt="">
                                    <div class="name-location">
                                        <strong>Shadab Alam</strong>
                                        <span><i class="fa fa-map-marker"></i><a href="#">Delhi</a></span>
                                    </div>
                                    
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                

                
            </div>
        </div>
        <!-- End Profile Content -->
    </div>
</div>
<!--=== End Profile ===-->


<script type="text/javascript" src="/static/plugins/circles-master/circles.js"></script>
<script type="text/javascript" src="/static/plugins/sky-forms-pro/skyforms/js/jquery-ui.min.js"></script>
<script type="text/javascript" src="/static/plugins/scrollbar/js/jquery.mCustomScrollbar.concat.min.js"></script>

<script type="text/javascript" src="/static/js/plugins/circles-master.js"></script>
<script type="text/javascript">
    jQuery(document).ready(function () {
        App.initScrollBar();
        CirclesMaster.initCirclesMaster1();
    });
</script>
