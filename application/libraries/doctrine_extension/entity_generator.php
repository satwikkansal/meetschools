...
class Entity_Generator extends EntityGenerator {
    /**
     * @var bool
     */
...
';
    ...
    {
        $array = $this->_getClassNameAndNamespace($metadata->name);
    ...
    {
        $methodName = ($type != 'get' ? $type . '_' : '' ) . $fieldName;
    ...
    private function _generateAssociationMappingPropertyDocBlock(array $associationMapping, ClassMetadataInfo $metadata)
    {
        $lines = array();
        $lines[] = $this->_spaces . '/**';
        $lines[] = $this->_spaces . ' * @var ' . $associationMapping['targetEntity'] . ' $' . $associationMapping['fieldName'];
    ...
    private function _getClassNameAndNamespace($fullName)
    {
        return array(
            'className' => ($pos = strrpos($fullName, '\\')) ? substr($fullName, $pos + 1, strlen($fullName)) : $fullName,
    ...
    private function fix_class_name($fullName) {
        if(!isset($fullName)) {
            return $fullName;
        }
        $array = $this->_getClassNameAndNamespace($fullName);
        ...
        foreach ($toks as $tok) {
            ...
            $className .= strtolower($this->singularize($tok));
        }
        $className = str_replace(" ", "_", ucwords(strtr($className, "_-", "  ")));
        echo $namespace . '\\' . $className;
        return $namespace . '\\' . $className;
    }

    private function fixMetadata(ClassMetadataInfo &$metadata) {
        $metadata->name = $this->fix_class_name($metadata->name);
        ...
        foreach ($metadata->associationMappings as &$fieldMapping) {
            ...
            if (isset($fieldMapping['targetEntity'])) {
                $fieldMapping['targetEntity'] = $this->fix_class_name($fieldMapping['targetEntity']);
            }
            if (isset($fieldMapping['inversedBy'])) {
                $fieldMapping['inversedBy'] = $this->fix_class_name($fieldMapping['inversedBy']);
            }
            if (isset($fieldMapping['mappedBy'])) {
                $fieldMapping['mappedBy'] =  $this->fix_class_name($fieldMapping['mappedBy']);
            }
        }
    }
}

/* End of file entity_generator.php */
/* Location: ./application/libraries/doctrine_extension/entity_generator.php */
